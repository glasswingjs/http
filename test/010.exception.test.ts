...
import {HttpException, HttpResponseCode, HttpResponseMessage, UnauthorizedException} from '../src'
...
describe('src/exception => X extends HttpException', () => {
  it('::toString()', () => {
    try {
      ...
      expect(ex.code).to.equal(HttpResponseCode.UNAUTHORIZED)
      expect(ex.message.code).to.equal(HttpResponseCode.UNAUTHORIZED)
      expect(ex.message.error).to.equal(HttpResponseMessage.UNAUTHORIZED)
